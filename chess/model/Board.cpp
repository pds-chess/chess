...
void Board::movePiece(Piece* target_piece, const Coordinates& final_coords) {
  ...
  if (pawn != nullptr && pawn->validateEnPassant(final_coords) == true) {
    ...
    removePiece(
        Coordinates(final_coords.getRow() + mult, final_coords.getCol()));
  } else {
    target_piece->movePiece(final_coords);
    if (isCapture(target_piece, final_coords)) removePiece(final_coords);
  }
  update();
}

bool Board::isCapture(Piece* target_piece,
                      const Coordinates& final_coords) const {
...
std::list<Piece*> Board::getPieces() const { return pieces_; }

void Board::createPiece(const Coordinates& coords, Color color,
                        PieceType type) {
  if (getPiece(coords) != nullptr) {
    throw std::logic_error(
        "Não é possível criar uma peça em um espaço ocupado.");
  }
