...
Rook::Rook(const Coordinates& initial_coords, Color color, const Board& board)
...
bool Rook::validateMove(const Coordinates& final_coordinates) const {
  ...
  if (RowP == RowF || ColP == ColF) {
    ...
    for (int i = 1, j = 1;
         RowP + i * multiplier_i < 8 && ColP + j * multiplier_j < 8; i++, j++) {
      Piece* PieceF = getBoard().getPiece(
          Coordinates(RowP + i * multiplier_i, ColP + j * multiplier_j));
      if (PieceF == nullptr) {
      ...
      if (PieceF != nullptr)
        if (PieceF->getColor() != getColor() &&
            RowP + i * multiplier_i == RowF &&
            ColP + j * multiplier_j == ColF) {
          return true;
        }
      if (PieceF != nullptr) return false;
    }
  }
  return false;
}

bool Rook::hasMoved() const { return moved_; }

char Rook::getPieceChar() const { return 'R'; }
