...
King::King(const Coordinates& initial_coords, Color color, const Board& board)
...
void King::movePiece(const Coordinates& final_coordinates) {
  if (!hasMoved() &&
      (final_coordinates.getCol() == 6 || final_coordinates.getCol() == 2))
...
bool King::validateMove(const Coordinates& final_coordinates) const {
  ...
  if (DeltaCol <= 1 && DeltaRow <= 1) {
    Piece* PieceF = getBoard().getPiece(
        Coordinates(final_coordinates.getRow(), final_coordinates.getCol()));
    if (PieceF == nullptr || PieceF->getColor() != getColor()) return true;
  }
...
bool King::hasMoved() const { return moved_; }

bool King::short_castle(const Coordinates& final_coordinates) const {
  if (final_coordinates.getCol() == getCoords().getCol() + 2) return true;
  if (final_coordinates.getCol() == getCoords().getCol() - 2) return false;
}
...
bool King::validateCastle(const Coordinates& final_coordinates) const {
  ...
  if (short_castle(final_coordinates) &&
      dynamic_cast<Rook*>(pieceCol_7) != NULL) {
    rook = dynamic_cast<Rook*>(pieceCol_7);
    if (rook->hasMoved())
      return false;
    else {
      if (getBoard().getPiece(Coordinates(
              getCoords().getRow(), getCoords().getCol() + 1)) == nullptr &&
          getBoard().getPiece(Coordinates(getCoords().getRow(),
                                          getCoords().getCol() + 2)) == nullptr)
        return true;
      else
        return false;
    }
  ...
  if (!short_castle(final_coordinates) &&
      dynamic_cast<Rook*>(pieceCol_0) != NULL) {
    ...
    if (rook->hasMoved())
      ...
      if (getBoard().getPiece(Coordinates(
              getCoords().getRow(), getCoords().getCol() - 1)) == nullptr &&
          getBoard().getPiece(Coordinates(
              getCoords().getRow(), getCoords().getCol() - 2)) == nullptr &&
          getBoard().getPiece(Coordinates(getCoords().getRow(),
                                          getCoords().getCol() - 3)) == nullptr)
        return true;
      else
        return false;
    }
...
void King::castle(const Coordinates& final_coordinates) {
  ...
  if (short_castle(final_coordinates)) {
    rook = dynamic_cast<Rook*>(pieceCol_7);
    rook->movePiece(Coordinates(rook->getCoords().getRow(),
                                rook->getCoords().getCol() - 2));
    setCoords(final_coordinates);
  } else {
    rook = dynamic_cast<Rook*>(pieceCol_0);
    rook->movePiece(Coordinates(rook->getCoords().getRow(),
                                rook->getCoords().getCol() + 3));
    setCoords(final_coordinates);
  }
}

char King::getPieceChar() const { return 'K'; }
...
bool King::isCheck() {
  ...
  if (kingColor == White) {
    pawnMoves[0][0] = -1;
    pawnMoves[0][1] = -1;  // Peão branco atacando para cima e para a esquerda
    pawnMoves[1][0] = -1;
    pawnMoves[1][1] = 1;  // Peão branco atacando para cima e para a direita
  } else {
    pawnMoves[0][0] = 1;
    pawnMoves[0][1] = -1;  // Peão preto atacando para baixo e para a esquerda
    pawnMoves[1][0] = 1;
    pawnMoves[1][1] = 1;  // Peão preto atacando para baixo e para a direita
  }
