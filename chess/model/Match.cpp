...
std::string Match::boardToString() const { return board_.boardToString(); }
...
void Match::movePiece(int row_start, int col_start, int row_end, int col_end) {
  ...
  if (board_.isCapture(target_piece, final_coords)) {
    ...
    if (pawn != nullptr && pawn->validateEnPassant(final_coords) == true) {
      ...
      end_piece = board_
                      .getPiece(Coordinates(final_coords.getRow() + mult,
                                            final_coords.getCol()))
                      ->getType();
    } else
...
bool Match::isDraw() const {
  for (auto item : current_turn_->getPieces()) {
    for (int i = 0; i < 8; i++) {
      for (int j = 0; j < 8; j++) {
        if (item->getCoords().getRow() == i &&
            item->getCoords().getCol() == i) {
          continue;
        }
        try {
          if (simulateMove(Coordinates(item->getCoords().getRow(),
                                       item->getCoords().getCol()),
                           Coordinates(i, j))) {
            return false;
          }
...
bool Match::simulateMove(const Coordinates& coord_start,
                         const Coordinates& coord_end) const {
...
void Match::registerMove(Piece* target_piece, Coordinates start_coords,
                         Coordinates final_coords) {
  Move move(start_coords, final_coords, current_turn_->getplayerColor(),
            current_turn_->getPlayerName(), game_id_);
...
std::string Match::showCapturedPieces() {
  ...
  for (PieceType piece : current_turn_->getCapturedPieces()) {
    if (capturedCount.find(piece) != capturedCount.end()) {
      ...
      capturedCount[piece] = 1;
  }

  // Imprimimos o resumo das peças capturadas
  for (const auto& entry : capturedCount) {
    capturedPieces += "\n" + std::to_string(entry.second) + "x ";
    if (entry.first == ROOK) capturedPieces += "Rook";
    if (entry.first == PAWN) capturedPieces += "Pawn";
    if (entry.first == KNIGHT) capturedPieces += "Knight";
    if (entry.first == BISHOP) capturedPieces += "Bishop";
    if (entry.first == QUEEN) capturedPieces += "Queen";
    if (entry.first == KING) capturedPieces += "King";
    if (entry.second > 1) capturedPieces += "s";
    capturedPieces += "\n";
  }
